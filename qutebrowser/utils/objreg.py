...
class ObjectRegistry(collections.UserDict):
    ...
    def __init__(self) -> None:
        super().__init__()
        self._partial_objs = {
        }  # type: MutableMapping[_IndexType, Callable[[], None]]
        self.command_only = []  # type: MutableSequence[str]
...
def _get_tab_registry(win_id: _WindowTab,
    ...
    if tab_id == 'current' and win_id is None:
        window = QApplication.activeWindow()  # type: Optional[QWidget]
...
def _get_window_registry(window: _WindowTab) -> ObjectRegistry:
    ...
    try:
        if window == 'current':
            win = QApplication.activeWindow()  # type: Optional[QWidget]
